...
server.post("/api/stories", { preHandler: authenticate }, async (request: any, reply) => {
...
server.get("/api/stories", { preHandler: authenticate }, async (request: any) => {
  const stories = listStories(Number(request.user.sub), 20);
  return stories.map(storyToResponse);
});

server.get("/api/stories/:id", { preHandler: authenticate }, async (request: any, reply) => {
...
server.get("/api/stories/:id/audio", { preHandler: authenticate }, async (request: any, reply) => {
  const story = getStoryById(request.params.id, Number(request.user.sub));
  if (!story?.audio_path) {
